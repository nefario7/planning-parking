...
void Environment::create_map(const string file_name) {
    ifstream input_file;
...
void Environment::create_parking_costmap(const string file_name) {
    ifstream input_file;
    input_file.open(file_name, ios::in);

    if (!input_file.is_open()) {
        cout << "Error opening parking map file" << endl;
        return;
    }

    cout << "Creating parking map from : " << file_name << endl;
    string line = "";
    ...
    while (getline(input_file, line)) {
        ...
        while (getline(lineStream, temp_str, ','))
            row.push_back(stod(temp_str));

        parking_costmap.push_back(row);
        row.clear();
    }
}
...
void Environment::create_primitives(const string file_name) {
    ...
    if (!input_file.is_open()) {
        cout << "Error opening primitives file" << endl;
        return;
    }
